...
class AUS_MOPSO_Parallel:
    def __init__(self, data_dir, model_path, bounds, num_particles=50, max_iter=100,
                 w=0.7, c1=1.5, c2=1.5, fast_mode=False, model_type='standard'):
        self.data_dir = data_dir
        self.bounds = bounds
        ...
        if model_type == 'standard':
            ...
            self.model_class = Optimized_Interaction_Network
...
if __name__ == "__main__":
    ...
    MODEL_PATH = os.path.join(_MODEL_DIR, "standard_agent.pth")
    ...
    save_path = os.path.join(_RESULT_DIR, "MOPSO_Pareto_Results_Standard.xlsx")
    ...
    ax1.set_title('Standard Agent Optimization Process')

    lines = [l1, l2]
    ax1.legend(lines, [l.get_label() for l in lines], loc='center right')
    ax1.grid(True, alpha=0.3)

    axes[1].scatter(df_res['ALCC'], df_res['NTI'], c='purple', edgecolor='k', s=50, label='Pareto Optimal (Standard)')
    axes[1].set_xlabel('Annualized Life-Cycle Cost (CNY)')
    axes[1].set_ylabel('Nutrient Transport Index (NTI)')
    axes[1].set_title('Pareto Frontier (Standard Agent)')
