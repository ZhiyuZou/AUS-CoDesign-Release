...
class Optimized_Interaction_Network(nn.Module):
    ...
    def forward(self, x):
        ...
        scale_raw, shift_raw = torch.chunk(stat_raw, 2, dim=1)  # [Batch, 512], [Batch, 512]
...
class Improved_DuelingQR_DQNAgent:
    ...
    def select_action(self, states, masks=None, is_train=True):
        ...
        if is_train:
            ...
            for idx in random_indices:
                if masks is not None:
                    ...
                    actions[idx] = np.random.choice(valid_actions) if len(valid_actions) > 0 else 0
                else:
                    actions[idx] = np.random.randint(0, self.action_dim)

            greedy_indices = np.where(random_vals >= epsilon)[0]
            if len(greedy_indices) > 0:
                greedy_states = torch.tensor(states[greedy_indices], dtype=torch.float32, device=self.device)
                with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                selected = q_expect.argmax(dim=1).cpu().numpy()
            return selected[0] if batch_size == 1 else selected
    ...
    def learn(self):
        ...
        with torch.no_grad():
            ...
            next_q_expect[next_mask_batch == 0] = -float('inf')  # Masking Target
...
class AUS_Environment:
    ...
    def calculate_statistics(self):
        ...
        e_mean_est = (self.config_ranges["e_max"][1] + self.config_ranges["e_max"][0]) / 2 / 2

        dyn_mean = np.array([
            e_mean_est,
            np.mean(all_data[:, 0]),  # T
            np.mean(all_data[:, 1]),  # I
            np.mean(all_data[:, 2]),  # Z
            np.mean(all_data[:, 3]),  # u
            self.total_time / 2  # step
        ], dtype=np.float32)
    ...
    def step(self, action: int):
        ...
        info = {
            ...
            "current_config": self.current_config, "action_mask": next_mask
        }
...
def train(data_dir, target_episodes=5000, seed=0):
    ...
    print(f"=== Training proposed (single env, batch=256, 32 sub-buffers) | {timestamp} ===")
    ...
    for ep in range(1, target_episodes + 1):
        ...
        if ep % 2500 == 0:
            torch.save(agent.current_net.state_dict(),
                       os.path.join(_MODEL_DIR, f"proposed_seed{seed}_checkpoint_ep{ep}.pth"))
        pd.DataFrame(train_log).to_excel(
            os.path.join(_RESULT_DIR, f"proposed_seed{seed}_log_{timestamp}.xlsx"), index=False)

    pd.DataFrame(train_log).to_excel(
        os.path.join(_RESULT_DIR, f"proposed_seed{seed}_log_{timestamp}.xlsx"), index=False)
    torch.save(agent.current_net.state_dict(),
               os.path.join(_MODEL_DIR, f"proposed_seed{seed}_final_{timestamp}.pth"))
    print(f"Saved final proposed model and training log.")
