...
class AUS_Environment:
    ...
    def load_all_year_data(self):
        for year in self.train_years:
            file_path = f"{self.data_dir}/{year}.xlsx"
            try:
    ...
    def _random_config(self) -> dict:
        config = {}
        config["p_az"] = np.random.uniform(*self.config_ranges["p_az"])
        config["e_max"] = np.random.uniform(*self.config_ranges["e_max"])
        config["m"] = np.random.randint(*self.config_ranges["m"])
        config["q0"] = np.random.uniform(*self.config_ranges["q0"])
        return config

    def reset(self, random_year: bool = True, fixed_config: Optional[dict] = None):
        if fixed_config is not None:
            self.current_config = fixed_config.copy()
            self.current_config["q0"] = np.clip(
                self.current_config.get("q0", 0.002),
                self.config_ranges["q0"][0],
                self.config_ranges["q0"][1]
            )
        else:
        ...
        self.current_year = random.choice(self.train_years) if random_year else self.train_years[0]
    ...
    def normalize_state(self, state: np.ndarray) -> np.ndarray:
        mean_dynamic = np.array([
            (self.E_max + self.E_min) / 2,
            np.mean([self.year_data_dict[y][:, 0] for y in self.train_years]),
            np.mean([self.year_data_dict[y][:, 1] for y in self.train_years]),
            np.mean([self.year_data_dict[y][:, 2] for y in self.train_years]),
            np.mean([self.year_data_dict[y][:, 3] for y in self.train_years]),
            self.total_time / 2
        ], dtype=np.float32)
        std_dynamic = np.array([
            20.0,
            np.std([self.year_data_dict[y][:, 0] for y in self.train_years]),
            np.std([self.year_data_dict[y][:, 1] for y in self.train_years]),
            np.std([self.year_data_dict[y][:, 2] for y in self.train_years]),
            np.std([self.year_data_dict[y][:, 3] for y in self.train_years]),
            self.total_time / 4
        ], dtype=np.float32)

        config_mean = np.array([
            np.mean(self.config_ranges["p_az"]),
            np.mean(self.config_ranges["e_max"]),
            np.mean(self.config_ranges["m"]),
            np.mean(self.config_ranges["q0"])
        ], dtype=np.float32)
        config_std = np.array([
            (self.config_ranges["p_az"][1] - self.config_ranges["p_az"][0]) / 10,
            (self.config_ranges["e_max"][1] - self.config_ranges["e_max"][0]) / 10,
            (self.config_ranges["m"][1] - self.config_ranges["m"][0]) / 10,
            (self.config_ranges["q0"][1] - self.config_ranges["q0"][0]) / 10
        ], dtype=np.float32)

        mean = np.concatenate([mean_dynamic, config_mean])
        std = np.concatenate([std_dynamic, config_std])
        std[std < 1e-6] = 1e-6
    ...
    def _map_action_to_compressors(self, action: int) -> int:
        open_ratio = action / (self.fixed_action_dim - 1)
        actual_m = round(open_ratio * self.M)
        return max(0, min(actual_m, self.M))
    ...
    def step(self, action: int):
        ...
        reward = 0.0
        self.current_step += 1
        u_next = self.env_data[self.current_step, 3] if self.current_step < 2879 else u_t
        ...
        if energy_surplus > 0:
            ...
            actual_compressors = 0
            q_t = 0
            total_load = self.b
            energy_surplus_new = g_t - total_load
            if energy_surplus_new > 0:
                c_t = min(energy_surplus_new, self.c_max, (self.E_max - E_t) / (self.eta_c * self.dt))
                E_next = E_t + self.eta_c * c_t * self.dt
            else:
                E_next = E_t + energy_surplus_new * self.dt / self.eta_d
            flag = 0
        ...
        raw_next_state = np.array([
            E_next, T_t, I_t, Z_t, u_t, self.current_step,
            self.P_AZ, self.E_max, self.M, self.Q0
        ], dtype=np.float32)
        next_state_norm = self.normalize_state(raw_next_state)
        self.current_state = next_state_norm

        info = {
            "Vt": Vt, "f2_t": f2_t, "energy_waste": energy_waste,
            "solar_power": g_t, "current_year": self.current_year, "current_step": self.current_step,
            "c_t": c_t, "total_load": total_load,
            "current_config": self.current_config,
            "actual_compressors": actual_compressors,
            "action_ratio": open_ratio,
            "flag": flag
        }
        return next_state_norm, reward, done, info

# ------------------------------------------------------
# ------------------------------------------------------
class DuelingQR_DQNNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        value_quantiles = self.value_stream(features).unsqueeze(1)
        advantage_quantiles = self.advantage_stream(features).view(-1, self.action_dim, self.num_quantiles)
        q_quantiles = value_quantiles + advantage_quantiles - advantage_quantiles.mean(dim=1, keepdim=True)
        return q_quantiles

    def get_expectation(self, quantiles):
        return torch.mean(quantiles, dim=-1)

class DuelingQR_DQNAgent:
    def __init__(self, state_dim, action_dim=24, num_quantiles=200):
        ...
        self.current_net = DuelingQR_DQNNetwork(state_dim, action_dim, num_quantiles).to(self.device)
        self.target_net = DuelingQR_DQNNetwork(state_dim, action_dim, num_quantiles).to(self.device)
    ...
    def select_action(self, state, is_train=True):
        ...
        if is_train:
            epsilon = self.epsilon_end + (self.epsilon_start - self.epsilon_end) * np.exp(
                -self.train_steps / self.epsilon_decay)
            ...
            if batch_size == 1: return final_actions[0]
            return final_actions
        else:
            with torch.no_grad():
                quantiles = self.current_net(state_tensor)
                q_expect = self.current_net.get_expectation(quantiles)
            action = q_expect.argmax(dim=1).cpu().numpy()
            if batch_size == 1: return action[0]
            return action
    ...
    def learn(self):
        ...
        with torch.no_grad():
            ...
            next_quantiles_selected = next_quantiles.gather(
                1, next_action.unsqueeze(2).expand(-1, -1, self.num_quantiles)
            ).squeeze(1)
        ...
        current_quantiles_selected = current_quantiles.gather(
            1, action_batch.unsqueeze(2).expand(-1, -1, self.num_quantiles)
        ).squeeze(1)
...
def train(data_dir, target_episodes=5000, seed=0):
    ...
    print(f"=== Training standard (single env, batch=256, 32 sub-buffers) | {timestamp} ===")
    env = AUS_Environment(data_dir)
    agent = DuelingQR_DQNAgent(env.dynamic_dim + env.static_dim, env.fixed_action_dim)
    train_log = []
    ...
    for ep in range(1, target_episodes + 1):
        ...
        if ep % 2500 == 0:
            torch.save(agent.current_net.state_dict(),
                       os.path.join(_MODEL_DIR, f"standard_seed{seed}_checkpoint_ep{ep}.pth"))
        pd.DataFrame(train_log).to_excel(
            os.path.join(_RESULT_DIR, f"standard_seed{seed}_log_{timestamp}.xlsx"), index=False)

    pd.DataFrame(train_log).to_excel(
        os.path.join(_RESULT_DIR, f"standard_seed{seed}_log_{timestamp}.xlsx"), index=False)
    torch.save(agent.current_net.state_dict(),
               os.path.join(_MODEL_DIR, f"standard_seed{seed}_final_{timestamp}.pth"))
    print(f"Saved final standard model and training log.")
