...
class MaskedStandardAgent:
    ...
    def select_action(self, state, masks=None, is_train=True):
        ...
        if is_train:
            ...
            random_indices = np.where(random_vals < epsilon)[0]
            if len(random_indices) > 0:
                for idx in random_indices:
                    if masks is not None:
                        valid_actions = np.where(masks[idx] == 1)[0]
                        if len(valid_actions) > 0:
                            actions[idx] = np.random.choice(valid_actions)
                        else:
                            actions[idx] = 0
                    else:
                        actions[idx] = np.random.randint(0, self.action_dim)

            greedy_indices = np.where(random_vals >= epsilon)[0]
            if len(greedy_indices) > 0:
                greedy_states = state_tensor[greedy_indices]

                with torch.no_grad():
                    quantiles = self.current_net(greedy_states)
                    q_expect = self.current_net.get_expectation(quantiles)

                    if masks is not None:
                        greedy_masks = torch.tensor(masks[greedy_indices], dtype=torch.float32, device=self.device)
                        q_expect[greedy_masks == 0] = -float('inf')

                    actions[greedy_indices] = q_expect.argmax(dim=1).cpu().numpy()

            if batch_size == 1: return actions[0]
            return actions
        else:
            with torch.no_grad():
                ...
                q_expect = self.current_net.get_expectation(quantiles)
                if masks is not None:
                    mask_tensor = torch.tensor(masks, dtype=torch.float32, device=self.device)
                    q_expect[mask_tensor == 0] = -float('inf')

            action = q_expect.argmax(dim=1).cpu().numpy()
            if batch_size == 1: return action[0]
            return action

    def add_experience(self, state, action, reward, next_state, done, next_mask, group=0):
    ...
    def learn(self):
        ...
        next_mask_batch = torch.tensor(np.array([e[5] for e in batch]), dtype=torch.float32, device=self.device)

        with torch.no_grad():
            next_quantiles = self.target_net(next_state_batch)
            next_q_expect = self.target_net.get_expectation(next_quantiles)
            next_q_expect[next_mask_batch == 0] = -float('inf')
...
class AUS_Environment:
    ...
    def load_all_year_data(self):
        ...
        for year in self.train_years:
            try:
                npy_path = os.path.join(self.data_dir, f"{year}.npy")
                xlsx_path = os.path.join(self.data_dir, f"{year}.xlsx")

                if os.path.exists(npy_path):
                    self.year_data_dict[year] = np.load(npy_path)
                    loaded_count += 1
    ...
    def calculate_statistics(self):
        ...
        dyn_mean = np.array([
            e_mean_est,
            np.mean(all_data[:, 0]), np.mean(all_data[:, 1]),
        ...
        dyn_std = np.array([
            20.0,
            np.std(all_data[:, 0]), np.std(all_data[:, 1]),
            np.std(all_data[:, 2]), np.std(all_data[:, 3]),
            self.total_time / 4
        ], dtype=np.float32)

        stat_mean = np.array([
            np.mean(self.config_ranges["p_az"]), np.mean(self.config_ranges["e_max"]),
            np.mean(self.config_ranges["m"]), np.mean(self.config_ranges["q0"])
        ], dtype=np.float32)
    ...
    def _get_action_mask(self, E_t: float, I_t: float) -> np.ndarray:
        ...
        total_power = solar_power + max_discharge
    ...
    def step(self, action: int):
        ...
        if self.current_step < self.total_time:
            u_next = self.env_data[self.current_step, 3]
        else:
            u_next = u_t
        ...
        info = {
            ...
            "action_mask": next_mask, "current_config": self.current_config
        }
...
def train(data_dir, target_episodes=5000, seed=0):
    ...
    print(f"=== Training masking (single env, batch=256, 32 sub-buffers) | {timestamp} ===")
    env = AUS_Environment(data_dir)
    agent = MaskedStandardAgent(env.dynamic_dim + env.static_dim, env.fixed_action_dim)
    train_log = []

    for ep in range(1, target_episodes + 1):
        state, mask = env.reset(random_year=True)
        done = False
        ep_reward = ep_nti = ep_waste = 0.0
        loss_sum, loss_n = 0.0, 0
        while not done:
            action = int(agent.select_action(state[None, :], mask[None, :], is_train=True))
            next_state, reward, done, info = env.step(action)
            next_mask = info['action_mask']
            group = config_group(env.current_config, env.config_ranges)
            agent.add_experience(state, action, reward, next_state, done, next_mask, group)
        ...
        if ep % 2500 == 0:
            torch.save(agent.current_net.state_dict(),
                       os.path.join(_MODEL_DIR, f"masking_seed{seed}_checkpoint_ep{ep}.pth"))
        pd.DataFrame(train_log).to_excel(
            os.path.join(_RESULT_DIR, f"masking_seed{seed}_log_{timestamp}.xlsx"), index=False)

    pd.DataFrame(train_log).to_excel(
        os.path.join(_RESULT_DIR, f"masking_seed{seed}_log_{timestamp}.xlsx"), index=False)
    torch.save(agent.current_net.state_dict(),
               os.path.join(_MODEL_DIR, f"masking_seed{seed}_final_{timestamp}.pth"))
    print(f"Saved final masking model and training log.")
